...
public class RNHmsInstanceId extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void getToken(final String appId, final String tokenScope, Callback callBack) {
        try {
            String token = HmsInstanceId.getInstance(ActivityUtils.getRealActivity(getCurrentActivity(), mContext)).getToken(appId, tokenScope);
    ...
    @ReactMethod
    public void deleteToken(final String appId, final String tokenScope, Callback callBack) {
        try {
            HmsInstanceId.getInstance(ActivityUtils.getRealActivity(getCurrentActivity(), mContext)).deleteToken(appId, tokenScope);
            callBack.invoke(Constants.RESULT_SUCESS);
        } catch (ApiException e) {
            callBack.invoke(e.getStatusCode(), e.getMessage());
        }
